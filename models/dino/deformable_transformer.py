...
class DeformableTransformer(nn.Module):
    ...
    def select_query_with_position_4(self, level_start_index, output_memory, enc_outputs_class_unselected, spatial_shapes, topk, num=2):
        ...
        data_topk_v = torch.gather(topk_data_sort, 1, topk_ps.unsqueeze(dim=2).repeat(1, 1, 
                                                    topk_data_sort.shape[2])) 

        indice_sort = torch.gather(id_sort_value, 1, topk_ps) 
        value_sort = torch.gather(p_value_sort_origin, 1, topk_ps)  
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None):
        ...
        if self.two_stage_type =='standard':
            ...
            if self.embed_init_tgt:
                ...
                tgt_undetach = torch.gather(output_memory, 1, topk_proposals.unsqueeze(-1).repeat(1, 1, self.d_model))
                tgt_undetach_bias = self.activation_gelu(self.bias_norm(tgt_undetach.detach()))
                tgt_undetach_bias = self.head_tgt_bias(tgt_undetach_bias)
                tgt_undetach_bias_4 = self.tgt_bias_box(tgt_undetach_bias)
        ...
        return hs, references, hs_enc, gcs_transformer_out, gcs_back_out, gqs_enc, ref_gqs_enc, ref_enc, init_box_proposal, tgt_undetach_bias_4, level_start_index, spatial_shapes, topk_proposals
