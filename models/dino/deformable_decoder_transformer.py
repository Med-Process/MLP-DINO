...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None):
        """
        Input:srcs, masks, input_query_bbox, poss,input_query_label,attn_mask
            - srcs: List of multi features [bs, ci, hi, wi]
            - masks: List of multi masks [bs, hi, wi]
            - refpoint_embed: [bs, num_dn, 4]. None in infer
            - pos_embeds: List of multi pos embeds [bs, ci, hi, wi]  (原始的图像位置编码，sin)
...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    def forward_ffn(self, tgt):
        ...
        # tgt = tgt + self.dropout4(F.sigmoid(tgt2) * tgt)
        tgt = self.norm3(tgt)
        return tgt
...
def build_deformable_decoder_transformer(args):
    decoder_query_perturber = None
