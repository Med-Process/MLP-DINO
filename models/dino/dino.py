...
class DINO(nn.Module):
    ...
    def forward(self, samples: NestedTensor, targets:List=None):
        ...
        hs, reference, hs_enc, gcs_transformer_out, gcs_back_out, gqs_enc, ref_gqs_enc, ref_enc, init_box_proposal, tgt_undetach_bias_4, level_start_index, spatial_shapes, topk_proposals = self.transformer(srcs, masks, input_query_bbox, poss,input_query_label,attn_mask)
...
class SetCriterion(nn.Module):
    ...
    def reweight_loss(self, targets_boxes):
        ...
        weight = (-torch.log(area_value) + 1e-5)
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        if self.REWEIGHT:
            ...
            loss_ce = sigmoid_focal_loss_reweight(src_logits, target_classes_onehot, num_boxes, new_weight, alpha=self.focal_alpha, gamma=2) * src_logits.shape[1]
        else: 
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        if self.REWEIGHT:
            new_weight = self.reweight_loss(target_boxes)
            loss_bbox[:, 2:] = new_weight.unsqueeze(dim=1) * loss_bbox[:, 2:]
